...
const ExplainStepInputSchema = z.object({
  step: z.enum(['checkinForm', 'flightDetails', 'boardingPass', 'securityQuestions', 'flightInfoBoard', 'counterStatusBoard']).describe("The specific step of the check-in process to explain."),
});
...
- **counterStatusBoard:** This screen is a supervisor's tool. Explain its purpose is to manage check-in resources. The supervisor can see which counters are open or closed, assign specific flights to counters, and assign agents to work at them. This is critical for managing passenger flow, ensuring flights have enough staff, and adapting to operational changes quickly.
`,
});
